...
def inicio_view(request):
    ...
    if hasattr(Voluntario, 'horas_aportadas'):
        ...
        chart_voluntarios_data = [getattr(v, 'horas_aportadas', 20) for v in voluntarios[:5]]
    else:
        chart_voluntarios_labels = [v.nombre for v in voluntarios[:5]]
        chart_voluntarios_data = [35, 28, 42, 15, 20][:len(chart_voluntarios_labels)]
    ...
    for act in actividades:
        fecha_str = act.fecha.strftime('%Y-%m-%d') if hasattr(act, 'fecha') and act.fecha else '2026-07-28'
...
def registro_publico_view(request):
    ...
    if request.method == 'POST':
        ...
        if tipo == 'Donante':
            ...
            cedula_val = cedula_ingresada if cedula_ingresada else f'VOL-{int(time.time())}'
            ...
            try:
                ...
                messages.error(request, f'La cédula {cedula_val} ya está registrada en el sistema.')
                return render(request, 'gestion/registro_publico.html', {'tipo': tipo})
            except Exception as e:
